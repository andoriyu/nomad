...
- `job` <code>(_deprecated_ string: "")</code> - Filter the list of evaluations
  to a specific job ID. Deprecated, use
  [`filter`](/api-docs/evaluations#filter) instead.

- `status` <code>(_deprecated_ string: "")</code> - Filter the list of
  evaluations to a specific evaluation status (one of `blocked`, `pending`,
  `complete`, `failed`, or `canceled`). Deprecated, use
  [`filter`](/api-docs/evaluations#filter) instead.
